...
void UExcelWorksheet::UpdateRuntimeColumnNames() const
{
	...
	if (mHeader.TableFormat == ExcelTableFormat::Normal)
	{
		return;
	}
...
bool UExcelWorksheet::HasValidDataTable() const
{
...
const UExcelWorksheetDataTable* UExcelWorksheet::SetRowStruct(const UScriptStruct* RowStruct)
{
	const UScriptStruct* currentStruct = GetRowStructInUsed();
	mHeader.RowStruct = RowStruct;
	const UScriptStruct* newStruct = GetRowStructInUsed();
...
const UExcelWorksheetDataTable* UExcelWorksheet::GetOrCreateDataTable()const
{
...
const UScriptStruct* UExcelWorksheet::GetRowStructInUsed()const
{
...
{
	return UTF8_TO_TCHAR(mData.title().c_str());
}

void UExcelWorksheet::SetTitle(FString title)
{
	UExcelWorksheet* findTitle = mWorkbook->SheetWithTitle(title);
	if (findTitle != nullptr)
	{
		if (findTitle == this)
		{
			return;
		...
		{
			UE_LOG(LogDirectExcel, Fatal, TEXT("Not allowed to set sheet title:%s duplicated with other sheets."), *title);
			return;
		}
	}
...
UExcelCell* UExcelWorksheet::CellAtFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	int32 columnIndex = FindColumnIndex(FName(columnName));
	if (columnIndex == -1)
	{
		return nullptr;
	}
	return CellAt(columnIndex, row);
}
...
bool UExcelWorksheet::ToBoolWithFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	UExcelCell* cell = CellAtFirstRowColumnName(columnName, row);
	if (cell == nullptr)
	{
		UE_LOG(LogDirectExcel, Warning, TEXT("Cannot find column:%s. You may need to SetTableHeader or SetTableFormat at first or provide a correct column"), *columnName);
		return false;
	}
	return cell->ToBool();
}

int32 UExcelWorksheet::ToIntWithFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	UExcelCell* cell = CellAtFirstRowColumnName(columnName, row);
	if (cell == nullptr)
	{
...
float UExcelWorksheet::ToFloatWithFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	UExcelCell* cell = CellAtFirstRowColumnName(columnName, row);
	if (cell == nullptr)
	{
...
FString UExcelWorksheet::ToStringWithFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	UExcelCell* cell = CellAtFirstRowColumnName(columnName, row);
	if (cell == nullptr)
	{
...
FDateTime UExcelWorksheet::ToDateTimeWithFirstRowColumnName(FString columnName, int32 row /*= 2*/) const
{
	UExcelCell* cell = CellAtFirstRowColumnName(columnName, row);
	if (cell == nullptr)
	{
		UE_LOG(LogDirectExcel, Warning, TEXT("Cannot find column:%s. You may need to SetTableHeader at first or provide a correct column"), *columnName);
		return FDateTime::MinValue();
	}
	return cell->ToDateTime();
}
