...
bool UExcelWorkbook::SaveAs(FString path, ExcelFileRelateiveDir relativeDir /*= ExcelFileRelateiveDir::ProjectSavedDir*/)
{
	...
	UE_LOG(LogDirectExcel, Log, TEXT("Success Save dataSize:%d to %s."), outExcelData.size(), *path);
	return true;
}
...
void UExcelWorkbook::InitActiveSheet()
{
	mActiveSheet = nullptr;
	xlnt::worksheet activeSheet = mData->active_sheet();
	if (!activeSheet.is_valid())
	{
		return;
	}
...
bool UExcelWorkbook::Reload(ExcelReloadStrategy reloadStrategy /*= ExcelReloadStrategy::ReloadWithOriginalTableHeaders*/)
{
	switch (reloadStrategy)
	{
	case ExcelReloadStrategy::ReloadAsNew:
		Unload();
		return Load(mPath);
		break;
	case ExcelReloadStrategy::ReloadWithOriginalTableHeaders:
	{
		//copy original table headers
		TMap<FName, FExcelTableHeader> tableHeaders = GetTableHeaders();
		Unload();
		if (Load(mPath))
		{
...
TMap<FName, FExcelTableHeader> UExcelWorkbook::GetTableHeaders() const
{
	TMap<FName, FExcelTableHeader> result;
	for (UExcelWorksheet* sheet : mSheets)
	{
		FString title = sheet->Title();
		FExcelTableHeader& refValue = result.FindOrAdd(FName(*title));
		refValue = sheet->GetTableHeader();
	}
	return result;
}

bool UExcelWorkbook::StartWatch(const UObject* WorldContextObject, FExcelWorkbookChangedDelegate onChanged, ExcelReloadStrategy reloadStrategy /*= ExcelReloadStrategy::ReloadWithOriginalTableHeader*/, float rate/*=1.f*/)
{
#ifdef WIN32
	...
	const wchar_t* p = *path;	//it have to be dir path
	...
	if (INVALID_HANDLE_VALUE == mFindHandle)
	{
		...
		UE_LOG(LogDirectExcel, Error, L"Failed to watch. Error:%d", errorCode);
		return false;
	}

	UWorld* world = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
	world->GetTimerManager().SetTimer(mWatchTimer, [this, WorldContextObject, onChanged, reloadStrategy]
		{
			...
			if (WAIT_OBJECT_0 == dwWait)
			{
				...
				do 
				{
					dwWait = ::WaitForSingleObject(mFindHandle, 0);
					if (!::FindNextChangeNotification(mFindHandle))
					{
						UE_LOG(LogDirectExcel, Warning, TEXT("Lost watch."));
						StopWatch(WorldContextObject);
...
#else
	UE_LOG(LogDirectExcel, Warning, TEXT("Not support StartWatch without Win32."));
#endif
...
bool UExcelWorkbook::StopWatch(const UObject* WorldContextObject)
{
	...
	{
		UWorld* world = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
		if (world == nullptr)	//may be in editor world
		{
			return false;
		}
...
TArray<FString> UExcelWorkbook::SheetTitles()const
{
	...
	{
		FString str = UTF8_TO_TCHAR(title.c_str());
		result.Add(MoveTemp(str));
	}
	return result;
}
...
void UExcelWorkbook::RemoveSheet(UExcelWorksheet* sheet)
{
	if (sheet == nullptr)
	{
		UE_LOG(LogDirectExcel, Fatal, TEXT("RemoveSheet: Parameter:sheet is null."));
		return;
	}
...
UExcelWorksheet* UExcelWorkbook::CopySheet(UExcelWorksheet* sheet, int32 index /*= -1*/)
{
	if (sheet == nullptr)
	{
		UE_LOG(LogDirectExcel, Fatal, TEXT("CopySheet Parameter:sheet is null."));
		return nullptr;
	}
...
{
	TArray<FString> result;
	for (const std::string& val : mData->custom_properties())
	{
		FString str = UTF8_TO_TCHAR(val.c_str());
		result.Add(MoveTemp(str));
	}
